...
import (
	...
	"errors"
	"fmt"
	"gitlab.ximalaya.com/x-fm/tail/pkg/ratelimiter"
	"gitlab.ximalaya.com/x-fm/tail/pkg/util"
	"gitlab.ximalaya.com/x-fm/tail/pkg/watch"
	"gopkg.in/tomb.v1"
	"io"
	"os"
	"strings"
	...
	for {
		...
		if err == nil {
			...
			if tail.Follow && line != "" {
				...
				err := tail.seekTo(SeekInfo{Offset: offset, Whence: io.SeekCurrent})
				if err != nil {
					tail.Kill(err)
					return
				}
...
func (tail *Tail) waitForChanges() error {
	if tail.changes == nil {
		...
		fmt.Printf("Change has happen pos :%d",pos)
		if err != nil {
			return err
		}
	...
	case <-tail.changes.Modified:
		...
		realOffset,_:=tail.file.Seek(0,io.SeekCurrent)
